...
| 目录扫描 | 用户指定目录：根目录 exe + 子目录"主程序"启发式挑选（同名优先 > 体积最大），内置安装器/反作弊/运行库/崩溃转储工具黑名单 |
| 跨源去重 | 同一款游戏被多个来源扫到时自动合并：保留信息更完整的一条，迁移游玩记录，被合并项写入忽略列表不再"复活" |
| 卸载失效标记 | 安装目录/主程序已不存在 → 标记"已卸载"（卡片灰显 + 角标），条目与游玩记录全部保留，可一键找回或按"已卸载"筛选 |
...
**测试**：`dotnet test`，共 30 个用例，都在 `tests/GameBuddy.Tests`：
...
- `GameDeduplicatorTests` —— 去重判定刻意保守，所以每个条件单独锁死：名称归一化、路径包含（含 `c://x` 与 `c:\x` 等价、大小写、尾斜杠）、"同一主程序"、"同名 + 路径重叠"、以及"不同游戏 / 不同盘同名不合并"的反例。
- `LibraryServiceMergeTests` —— 用内存版仓储跑完整扫描管道：合并后会话迁移到保留条目（累计时长不丢）、被合并项写进忽略列表、卸载标记只打在有路径证据的条目上。
...
- 跨源去重**刻意保守**：只在"同一 exe"或"名称相同且路径互相包含"时才合并。同一款游戏装在不同盘、或只有启动协议没有路径的条目，不会被自动合并。
- 卸载状态只在扫描时刷新；中途重装了要点一次"立即扫描"才会解除标记（启动时会按磁盘实际情况复核一次，所以不会误挡启动）。
- 单元测试覆盖了扫描、解析、去重与 `LibraryService` 管道；ViewModel（依赖 WPF 的 `ICollectionView`/`Dispatcher`）与主题切换还没有测试。
- 解决方案用的是 .NET 10 默认的 `.slnx` 格式，老版本 Visual Studio 可能只认 `.sln`。

## 7. 下一步路线（建议顺序）

1. **P1**：截图墙（读 Steam `screenshots` 目录）+ 统计页（时长排行、月度分布）。
2. **P1**：导入 Steam 本地时长（`localconfig.vdf`），补上"不是从本启动器启动"的会话。
3. **P2**：GOG / Xbox Game Pass 扫描源；右键菜单与批量操作。
4. **P2**：虚拟化列表 + 增量扫描 + 海报懒加载；补 ViewModel / 主题切换层测试。
5. **P2**：启动器自启动/最小化到托盘、游戏中浮窗（显示本次时长）。
6. **P2**：多封面来源与手动选图、封面本地文件拖拽；数据库（SQLite）替换 JSON 以支持更大规模。
7. **P2**：分组排序拖拽、标签自动补全、隐藏/家长锁。
8. **P3**：云同步库与统计数据、更多主题（玻璃拟态/复古 CRT）、插件市场。
